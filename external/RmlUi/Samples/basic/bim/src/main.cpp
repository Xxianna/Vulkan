...
#endif
{
	...
	if (!Backend::Initialize("BIM Sample", window_width, window_height, true))
	{
		Shell::Shutdown();
		return -1;
	}
	...
	// Load and show the BIM document.
	if (Rml::ElementDocument* document = context->LoadDocument("basic/bim/data/bim.rml"))
		document->Show();
