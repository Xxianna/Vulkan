...
public:
	...
	// Offscreen rendering mode: render to a VkImage instead of swapchain.
	// The output texture can be sampled by the host application.
	bool Initialize_Offscreen(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device,
...
private:
	...
	// Debug utility function pointers (loaded dynamically via vkGetDeviceProcAddr)
	PFN_vkSetDebugUtilsObjectNameEXT m_pfn_vkSetDebugUtilsObjectNameEXT = nullptr;
	PFN_vkCmdInsertDebugUtilsLabelEXT m_pfn_vkCmdInsertDebugUtilsLabelEXT = nullptr;
	VkImage m_offscreen_color_image = VK_NULL_HANDLE;
	VkImageView m_offscreen_color_image_view = VK_NULL_HANDLE;
	VmaAllocation m_offscreen_color_allocation = VK_NULL_HANDLE;
	VkFramebuffer m_offscreen_framebuffer = VK_NULL_HANDLE;
	VkRenderPass m_offscreen_render_pass = VK_NULL_HANDLE;
	VkSemaphore m_offscreen_render_complete_semaphore = VK_NULL_HANDLE;
	VkFence m_offscreen_fence = VK_NULL_HANDLE;
	VkFormat m_offscreen_format = VK_FORMAT_R8G8B8A8_UNORM;

	void Initialize_Offscreen_Resources(int width, int height) noexcept;
	void CreateOffscreenImage(int width, int height) noexcept;
	void CreateOffscreenRenderPass() noexcept;
	void CreateOffscreenFramebuffer() noexcept;
	void Destroy_Offscreen_Resources() noexcept;
};
