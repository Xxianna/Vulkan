...
### windows - vulkan - glfw
```bash
# cmake -B Build_VK -S . --preset samples -DRMLUI_BACKEND=GLFW_VK
cmake -B Build_VK -S . --preset samples -DRMLUI_BACKEND=GLFW_VK -DRMLUI_SVG_PLUGIN=ON  -DRMLUI_LOTTIE_PLUGIN=ON -DRMLUI_HARFBUZZ_SAMPLE=ON
...
```bash
# emcmake cmake -B Build_WebGL -S . --preset samples -DRMLUI_BACKEND=SDL_GL3 -DRMLUI_FONT_ENGINE=freetype
emcmake cmake -B Build_WebGL -S . --preset samples -DRMLUI_BACKEND=SDL_GL3 -DRMLUI_FONT_ENGINE=freetype -DRMLUI_SVG_PLUGIN=ON -DRMLUI_LOTTIE_PLUGIN=ON -DRMLUI_HARFBUZZ_SAMPLE=ON
emmake cmake --build Build_WebGL --parallel
```

## 效果示例

![alt text](<doc/屏幕截图 2026-06-23 111228.png>)
